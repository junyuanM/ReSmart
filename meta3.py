...
def plot_chart(chart_type, x_values, y_values, title, xlabel, ylabel, labels):
    # 使用Seaborn的调色板
    colors = sns.color_palette('pastel', len(y_values))
    ...
    if chart_type == 'bar':
        ...
        for i, (y, label, color) in enumerate(zip(y_values, labels, colors)):
            plt.bar(index + i * bar_width, y, bar_width, color=color, label=label)
        
        plt.xticks(index + (len(y_values) - 1) * bar_width / 2, x_values, fontsize=14)
        
        # # 添加数值标签
        # for i, (y, color) in enumerate(zip(y_values, colors)):
        #     for j, val in enumerate(y):
        #         plt.text(index[j] + i * bar_width, val + 1, str(val), ha='center', va='bottom')
...
def main():
    ...
    # plot_chart('line', range(100), [result[0][:100] for result in results], 'Regret', 'Time', 'Regret', labels)
    # plot_chart('bar', range(100), [result[1][:100] for result in results], 'Utility', 'time slot', 'Utility', labels)
    plot_chart('bar', range(10), [np.array(result[1])[np.linspace(0, num_time_slots-1, 10, dtype=int)] for result in results], 'Utility', 'time slot', 'Utility', labels)
    ...
    plot_chart('line', range(max_len), conflict_sums_interpolated, 'Average Conflict', 'time slot', 'Average Conflict', labels)
